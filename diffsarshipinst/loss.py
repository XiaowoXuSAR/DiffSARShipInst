...
from fvcore.nn import sigmoid_focal_loss_jit, poly_focal_loss_jit
import torchvision.ops as ops
from .util import box_ops
from .util.misc import get_world_size, is_dist_avail_and_initialized
from .util.box_ops import box_cxcywh_to_xyxy, box_xyxy_to_cxcywh, generalized_box_iou, distance_box_iou, complete_box_iou, \
alpha_box_iou, iou_distance_weight, mdp_box_iou, focused_iou
...
class SetCriterionDynamicK(nn.Module):
    """ This class computes the loss for DiffSARShipInst_ResNet50.
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        #########################################
        # self.balanced_l1_loss = BalancedL1Loss
        #########################################
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.DiffSARShipInst.ALPHA
            self.focal_loss_gamma = cfg.MODEL.DiffSARShipInst.GAMMA
        else:
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes,log=False):
        ...
        if self.use_focal or self.use_fed_loss:
            ...
            if self.use_focal:
                ...
                # cls_loss = poly_focal_loss_jit(src_logits, target_classes_onehot, alpha=self.focal_loss_alpha, gamma=self.focal_loss_gamma, reduction="none")
                ################################################
            else:
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(pred_box_list) != 0:
            ...
            # loss_bbox = self.balanced_l1_loss(src_boxes_norm, box_cxcywh_to_xyxy(target_boxes))
            # balanced_l1_loss
            # beta = 1.0
            # alpha = 0.5
            # gamma = 1.5
            # diff = torch.abs(src_boxes_norm - box_cxcywh_to_xyxy(target_boxes))
            ...
            ################################################
            loss_fiou = 1 - torch.diag(focused_iou(src_boxes, target_boxes_abs_xyxy))
            ################################################

            losses['loss_fiou'] = loss_fiou.sum() / num_boxes
        else:
            losses = {'loss_bbox': outputs['pred_boxes'].sum() * 0,
                      'loss_fiou': outputs['pred_boxes'].sum() * 0}
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        for batch_idx in range(batch_size):
            ...
            if len(mask_head_params)>0:
                ...
                img_h, img_w = mask_feat.size(1) * 2, mask_feat.size(2) * 2
                #########################################
                ...
                start = int(2 // 2)
                gt_masks = gt_masks[:, start::2, start::2]
                #########################################
...
class HungarianMatcherDynamicK(nn.Module):
    ...
    def __init__(self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1, cost_mask: float = 1, use_focal: bool = False):
        ...
        self.use_focal = use_focal
        self.use_fed_loss = cfg.MODEL.DiffSARShipInst.USE_FED_LOSS
        self.ota_k = cfg.MODEL.DiffSARShipInst.OTA_K
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.DiffSARShipInst.ALPHA
            self.focal_loss_gamma = cfg.MODEL.DiffSARShipInst.GAMMA
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(bs):
                ...
                ################################################
                cost_giou = -focused_iou(bz_boxes, bz_gtboxs_abs_xyxy)
                ################################################
...
def balanced_l1_loss(pred,
    ...
    Please see the `Libra_R-CNN <https://arxiv.org/pdf/1904.02701.pdf>`_
