...
def detector_postprocess(
    ...
    pred_global_masks = aligned_bilinear(masks.unsqueeze(1), 2)
    #########################################
...
class DiffSARShipInst(nn.Module):
    """
    Implement DiffSARShipInst_ResNet50
    """
    ...
    def __init__(self, cfg):
        ...
        self.num_classes = cfg.MODEL.DiffSARShipInst.NUM_CLASSES
        self.num_proposals = cfg.MODEL.DiffSARShipInst.NUM_PROPOSALS
        self.hidden_dim = cfg.MODEL.DiffSARShipInst.HIDDEN_DIM
        self.num_heads = cfg.MODEL.DiffSARShipInst.NUM_HEADS
        ...
        # build diffusion
        timesteps = 1000
        sampling_timesteps = cfg.MODEL.DiffSARShipInst.SAMPLE_STEP
        ...
        self.scale = cfg.MODEL.DiffSARShipInst.SNR_SCALE
        ...
        # Loss parameters:
        class_weight = cfg.MODEL.DiffSARShipInst.CLASS_WEIGHT
        fiou_weight = cfg.MODEL.DiffSARShipInst.FIOU_WEIGHT
        l1_weight = cfg.MODEL.DiffSARShipInst.L1_WEIGHT
        no_object_weight = cfg.MODEL.DiffSARShipInst.NO_OBJECT_WEIGHT
        self.deep_supervision = cfg.MODEL.DiffSARShipInst.DEEP_SUPERVISION
        self.use_focal = cfg.MODEL.DiffSARShipInst.USE_FOCAL
        self.use_fed_loss = cfg.MODEL.DiffSARShipInst.USE_FED_LOSS
        self.use_nms = cfg.MODEL.DiffSARShipInst.USE_NMS

        # Build Criterion.
        matcher = HungarianMatcherDynamicK(
            cfg=cfg, cost_class=class_weight, cost_bbox=l1_weight, cost_giou=fiou_weight, use_focal=self.use_focal
        )
        weight_dict = {"loss_ce": class_weight, "loss_bbox": l1_weight, "loss_fiou": fiou_weight}
    ...
    def model_predictions(self, backbone_feats, images_whwh, x, t, x_self_cond=None, clip_x_start=False):
        ...
        #########################################
        # self.box_shape.append((x_boxes[:, :, 2].cpu().numpy() - x_boxes[:, :, 0].cpu().numpy())[0, :] * \
        #                                 (x_boxes[:, :, 3].cpu().numpy() - x_boxes[:, :, 1].cpu().numpy())[0, :])
        # if len(self.box_shape) >= 232:
        #     box_shape = np.array(self.box_shape).reshape(-1, 1)
        #     box_shape = box_shape[box_shape != 0]
        #     print(box_shape.shape)
        #     print(np.max(box_shape))
        #     plt.figure(1)
        #     n, bins, patches = plt.hist(box_shape, bins=40, range=(0, 68000), edgecolor='white',
        #                                 align='left')
        #     for i in range(len(n)):
        #         plt.text(bins[i], n[i] * 1.02, int(n[i]), fontsize=5, horizontalalignment="center")
        #     plt.xlabel('Area of Ship')
